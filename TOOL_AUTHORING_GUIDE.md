...
## クリップボード出力の文字コード互換性
- コピーする文字列には全角チルダ `～`（U+FF5E）および波ダッシュ `〜`（U+301C）を含めない。文字化けを防ぐため、範囲表記は半角ハイフン `-` を用いる。
- コピー処理の直前に、必ず `text = text.replace(/[～〜]/g, '-');` を実行する。Clipboard API と `execCommand('copy')` のフォールバックの両方が、この置換済み文字列を使うことを確認する。
